...
function AddPaymentForm({ onSave, onCancel }: AddFormProps) {
  ...
  function handleSave() {
    if (tab === "card") {
      if (!cardName) { toast.error(t("profile.pagos.validation.name")); return; }
      if (!cardComplete.number || !cardComplete.expiry || !cardComplete.cvc) {
        toast.error(t("profile.pagos.validation.cardcomplete"));
        return;
      }
      if (!stripe || !elements) { toast.error(t("profile.pagos.validation.stripe")); return; }
      const cardElement = elements.getElement(CardNumberElement);
      if (!cardElement) { toast.error(t("profile.pagos.validation.form")); return; }
    ...
    if (tab === "paypal") {
      if (!ppEmail) { toast.error(t("profile.pagos.validation.paypal")); return; }
      onSave({ type: "paypal", label: ppLabel, isDefault: setAsDefault, paypalEmail: ppEmail });
    } else if (tab === "usdt") {
      if (!usdtAddress.trim()) { toast.error(t("profile.pagos.validation.usdt")); return; }
      onSave({
        type: "usdt", label: usdtLabel, isDefault: setAsDefault,
        cryptoNetwork: usdtNetwork.split(" ")[0], cryptoAddress: usdtAddress.trim(),
      });
    } else {
      if (!btcAddress.trim()) { toast.error(t("profile.pagos.validation.btc")); return; }
      onSave({
  ...
  const tabDefs: { id: FormTab; label: string; logo: React.ReactNode }[] = [
    { id: "card", label: t("profile.pagos.form.tab.card"), logo: <div className="flex items-center gap-1.5"><VisaLogo size={16} /><MastercardLogo size={18} /></div> },
    { id: "paypal", label: t("profile.pagos.form.tab.paypal"), logo: <PayPalLogo size={18} /> },
    { id: "usdt", label: t("profile.pagos.form.tab.usdt"), logo: <USDTLogo size={18} /> },
    { id: "btc", label: t("profile.pagos.form.tab.btc"), logo: <BTCLogo size={18} /> },
  ];
  ...
  return (
    <div className="border border-gray-200 rounded-xl overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-4 bg-gray-50 border-b border-gray-100">
        <p className="text-sm text-gray-900">{t("profile.pagos.form.title")}</p>
      ...
      <div className="p-5 space-y-5">
        ...
        {tab === "card" && (
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-2 text-xs text-gray-400">
              <span>{t("profile.pagos.form.card.brand")}</span>
            ...
            <div>
              <label className="block text-xs text-gray-400 mb-1.5">{t("profile.pagos.form.card.label")}</label>
              <input
                value={cardLabel}
                ...
                placeholder={t("profile.pagos.form.card.label.placeholder")}
              />
            </div>
            <div>
              <label className="block text-xs text-gray-400 mb-1.5">{t("profile.pagos.form.card.number")}</label>
            ...
            <div>
              <label className="block text-xs text-gray-400 mb-1.5">{t("profile.pagos.form.card.name")}</label>
              <input
                value={cardName}
                ...
                placeholder={t("profile.pagos.form.card.name.placeholder")}
              />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs text-gray-400 mb-1.5">{t("profile.pagos.form.card.expiry")}</label>
              ...
              <div>
                <label className="block text-xs text-gray-400 mb-1.5">{t("profile.pagos.form.card.cvv")}</label>
        ...
        {tab === "paypal" && (
          <div className="space-y-3">
            <div className="flex items-center gap-3 px-4 py-3 bg-sky-50/40 rounded-xl border border-sky-100">
              <PayPalLogo size={22} />
              <p className="text-xs text-gray-500">{t("profile.pagos.form.paypal.info")}</p>
            </div>
            <div>
              <label className="block text-xs text-gray-400 mb-1.5">{t("profile.pagos.form.paypal.label")}</label>
              <input
                value={ppLabel}
                onChange={(e) => setPpLabel(e.target.value)}
                className="w-full text-sm text-gray-900 border border-gray-200 rounded-lg px-3 py-2.5 focus:outline-none focus:border-gray-400 placeholder-gray-300"
                placeholder={t("profile.pagos.form.paypal.label.placeholder")}
              />
            </div>
            <div>
              <label className="block text-xs text-gray-400 mb-1.5">
                <Mail className="inline w-3 h-3 mr-1" strokeWidth={1.5} />
                {t("profile.pagos.form.paypal.email")}
              </label>
              <input
                type="email"
                value={ppEmail}
                ...
                placeholder={t("profile.pagos.form.paypal.email.placeholder")}
              />
            </div>
          </div>
        )}
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-3 px-4 py-3 bg-emerald-50/40 rounded-xl border border-emerald-100">
              ...
              <p className="text-xs text-gray-500 leading-relaxed">
                {t("profile.pagos.form.usdt.info")}
              </p>
            </div>
            <div>
              <label className="block text-xs text-gray-400 mb-1.5">{t("profile.pagos.form.usdt.label")}</label>
              <input
                value={usdtLabel}
                onChange={(e) => setUsdtLabel(e.target.value)}
                className="w-full text-sm text-gray-900 border border-gray-200 rounded-lg px-3 py-2.5 focus:outline-none focus:border-gray-400 placeholder-gray-300"
                placeholder={t("profile.pagos.form.usdt.label.placeholder")}
              />
            </div>
            <div>
              <label className="block text-xs text-gray-400 mb-1.5">{t("profile.pagos.form.usdt.network")}</label>
            ...
            <div>
              <label className="block text-xs text-gray-400 mb-1.5">{t("profile.pagos.form.usdt.address.suffix").replace("{network}", usdtNetwork.split(" ")[0])}</label>
              <input
                value={usdtAddress}
                ...
                placeholder={t("profile.pagos.form.usdt.address.placeholder")}
              />
            </div>
          </div>
        )}
        ...
        {tab === "btc" && (
          <div className="space-y-3">
            <div className="flex items-start gap-3 px-4 py-3 bg-orange-50/40 rounded-xl border border-orange-100">
              <BTCLogo size={18} />
              <p className="text-xs text-gray-500 leading-relaxed">
                {t("profile.pagos.form.btc.info")}
              </p>
            </div>
            <div>
              <label className="block text-xs text-gray-400 mb-1.5">{t("profile.pagos.form.btc.label")}</label>
              <input
                value={btcLabel}
                onChange={(e) => setBtcLabel(e.target.value)}
                className="w-full text-sm text-gray-900 border border-gray-200 rounded-lg px-3 py-2.5 focus:outline-none focus:border-gray-400 placeholder-gray-300"
                placeholder={t("profile.pagos.form.btc.label.placeholder")}
              />
            </div>
            <div>
              <label className="block text-xs text-gray-400 mb-1.5">{t("profile.pagos.form.btc.network")}</label>
              <div className="flex gap-2">
                {BTC_NETWORKS.map((n) => (
                  <button
                    key={n}
            ...
            <div>
              <label className="block text-xs text-gray-400 mb-1.5">{t("profile.pagos.form.usdt.address.suffix").replace("{network}", btcNetwork)}</label>
              <input
                value={btcAddress}
                ...
                placeholder={t("profile.pagos.form.btc.address.placeholder")}
              />
            </div>
            <div className="flex items-start gap-2">
              <AlertCircle className="w-3.5 h-3.5 text-amber-400 flex-shrink-0 mt-0.5" strokeWidth={1.5} />
              <p className="text-xs text-gray-400">{t("profile.pagos.form.btc.warning")}</p>
            </div>
          </div>
        )}
        ...
        <div className="flex items-center justify-between pt-2 border-t border-gray-100">
          <label className="flex items-center gap-2 cursor-pointer select-none">
            ...
            <span className="text-xs text-gray-600">{t("profile.pagos.form.default.label")}</span>
          </label>
          <div className="flex gap-2">
            <button
              type="button"
            ...
            >
              {t("profile.pagos.form.button.cancel")}
            </button>
            <button
              type="button"
              onClick={handleSave}
            ...
            >
              {isTokenizing ? t("profile.pagos.form.button.saving") : t("profile.pagos.form.button.save")}
            </button>
          </div>
        </div>
      </div>
...
export function ProfilePagos() {
  ...
  function handleSaveEdit(pm: PaymentMethod) {
    ...
    updatePaymentMethod(pm.id, updated)
      .then(() => { setEditingId(null); toast.success(t("profile.pagos.toast.updated")); })
      .catch(() => toast.error(t("profile.pagos.toast.updated.error")));
  }

  function handleAdd(pm: Omit<PaymentMethod, "id">) {
    addPaymentMethod(pm)
      .then(() => { setShowForm(false); toast.success(t("profile.pagos.toast.added")); })
      .catch(() => toast.error(t("profile.pagos.toast.added.error")));
  }

  function handleRemove(id: string) {
    removePaymentMethod(id)
      .then(() => toast.success(t("profile.pagos.toast.deleted")))
      .catch(() => toast.error(t("profile.pagos.toast.deleted.error")));
  }

  function handleSetDefault(id: string) {
    setDefaultPaymentMethod(id)
      .then(() => toast.success(t("profile.pagos.toast.default.updated")))
      .catch(() => toast.error(t("profile.pagos.toast.default.error")));
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base text-gray-900">{t("profile.pagos.title")}</h2>
          <p className="text-xs text-gray-400 mt-0.5">{t("profile.pagos.subtitle")}</p>
        </div>
        {!showForm && (
          <button
      ...
      {user.paymentMethods.length === 0 && !showForm && (
        <div className="flex flex-col items-center justify-center py-16 text-center bg-white border border-gray-100 rounded-xl">
          ...
          <p className="text-sm text-gray-600 mb-1">{t("profile.pagos.empty.title")}</p>
          <p className="text-xs text-gray-400 mb-5">{t("profile.pagos.empty.desc")}</p>
          <button
            onClick={() => setShowForm(true)}
            className="inline-flex items-center gap-1.5 text-xs text-gray-700 bg-gray-200 rounded-lg px-4 py-2.5 hover:bg-gray-300 transition-colors"
          >
            <Plus className="w-3.5 h-3.5" strokeWidth={2} />
            {t("profile.pagos.empty.button")}
          </button>
        </div>
      )}
                ...
                <div className="flex items-center gap-4 px-5 py-4">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {pm.isDefault && (
                        <span className={`text-[10px] px-2 py-0.5 rounded-full ${accent.badge}`}>
                          {t("profile.pagos.list.badge.default")}
                        </span>
                      )}
                    </div>
                    <p className="text-xs text-gray-400 mt-0.5">{cardTypeLabel(pm)}</p>
                    {pm.type === "card" && pm.cardExpiry && (
                      <p className="text-[11px] text-gray-400">{t("profile.pagos.list.expiry")} {pm.cardExpiry}</p>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <button
                      ...
                      title={pm.isDefault ? t("profile.pagos.list.badge.default") : t("profile.pagos.list.setdefault")}
                    ...
                    <button
                      ...
                      title={t("profile.pagos.list.expand")}
                ...
                {isExpanded && (
                  <div className={`px-5 pb-4 pt-3 border-t border-gray-50 ${accent.bg}`}>
                    {editingId === pm.id ? (
                      ...
                      <div className="space-y-3">
                        <div>
                          <label className="text-xs text-gray-400">{t("profile.pagos.list.cardholder")}</label>
                          <input
                            type="text"
                            value={editLabel}
                        ...
                        {pm.type === "paypal" && (
                          <div>
                            <label className="text-xs text-gray-400">{t("profile.pagos.list.email")}</label>
                            <input
                              type="email"
                              value={editEmail}
                        ...
                        {pm.type === "card" && (
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              <label className="text-xs text-gray-400">{t("profile.pagos.list.expiry.label")}</label>
                              <input
                                type="text"
                                value={editExpiry}
                            ...
                            <div>
                              <label className="text-xs text-gray-400">{t("profile.pagos.list.last4")}</label>
                        ...
                        <div className="flex items-center gap-2 pt-1">
                          ...
                          >
                            ...
                            {t("profile.pagos.form.button.save")}
                          </button>
                          <button
                            type="button"
                            onClick={cancelEdit}
                      ...
                      <>
                        {pm.type === "card" && (
                          <div className="grid grid-cols-2 gap-x-6 gap-y-1.5 text-xs">
                            <div><span className="text-gray-400">{t("profile.pagos.list.cardholder")}</span><p className="text-gray-700 mt-0.5">{pm.cardName}</p></div>
                            <div><span className="text-gray-400">{t("profile.pagos.list.last4")}</span><p className="text-gray-700 font-mono mt-0.5">···· {pm.cardLast4}</p></div>
                            <div><span className="text-gray-400">{t("profile.pagos.list.expiry.label")}</span><p className="text-gray-700 font-mono mt-0.5">{pm.cardExpiry}</p></div>
                            <div><span className="text-gray-400">{t("profile.pagos.list.network")}</span><p className="text-gray-700 mt-0.5 capitalize">{pm.cardBrand}</p></div>
                          </div>
                        )}
                        {pm.type === "paypal" && (
                          <div className="text-xs">
                            <span className="text-gray-400">{t("profile.pagos.list.email")}</span>
                            <p className="text-gray-700 mt-0.5">{pm.paypalEmail}</p>
                          </div>
                        )}
                        {(pm.type === "usdt" || pm.type === "btc") && (
                          <div className="space-y-1.5 text-xs">
                            <div><span className="text-gray-400">{t("profile.pagos.list.network")}</span><p className="text-gray-700 mt-0.5">{pm.cryptoNetwork}</p></div>
                            <div>
                              <span className="text-gray-400">{t("profile.pagos.list.address")}</span>
                        ...
                        {!pm.isDefault && (
                          ...
                          >
                            ...
                            {t("profile.pagos.list.setdefault")}
                          </button>
                        )}
                      </>
                    )}
