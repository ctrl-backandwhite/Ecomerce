...
function MiniProductCard({ product }: { product: Product }) {
  ...
  return (
    ...
    >
      ...
      <div className="p-3">
        <p className="text-xs text-gray-900 leading-snug mb-1.5 line-clamp-2 group-hover:text-gray-700 transition-colors">{product.name}</p>
    ...
    <div>
      ...
      <div className="flex gap-4 overflow-x-auto pb-3" style={{ scrollbarWidth: "thin" }}>
        ...
        >
          ...
          <span className="text-xs text-gray-400 group-hover:text-gray-700 transition-colors">Ver {category}</span>
        </Link>
      </div>
    </div>
  );
  ...
  return (
    <section className="py-16 bg-gray-50 border-t border-gray-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-12">
          <h2 className="text-3xl md:text-4xl tracking-tight text-gray-900 mb-2">Productos por Categoría</h2>
          <p className="text-gray-500">Explora nuestra selección completa</p>
        </div>
        <div className="flex flex-col gap-12">
          {CATEGORY_ORDER.map((cat) => (
            <CategoryRow key={cat} category={cat} products={products} />
          ))}
        </div>
      </div>
    </section>
