  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        >
          <FileText className="w-3.5 h-3.5" /> Nueva factura
        </button>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">
        <div className="grid grid-cols-[1.2fr_1fr_1.5fr_0.9fr_0.9fr_0.8fr_0.8fr_auto] gap-3 px-4 py-2.5 border-b border-gray-100 bg-gray-50/60">
          {["Nº Factura", "Nº Orden", "Cliente", "Emisión", "Vencimiento", "Total", "Estado", ""].map(h => (
        ...
        {filtered.map((inv, i) => {
          ...
          return (
            <div
              key={inv.id}
              className={`grid grid-cols-[1.2fr_1fr_1.5fr_0.9fr_0.9fr_0.8fr_0.8fr_auto] gap-3 px-4 py-3 items-center hover:bg-gray-50/60 transition-colors ${i !== filtered.length - 1 ? "border-b border-gray-50" : ""}`}
            >
              {/* Invoice number */}
              <p className="text-xs text-gray-900 font-mono">{inv.invoiceNumber}</p>

              {/* Order number */}
              <p className="text-xs text-gray-500 font-mono">{inv.orderNumber}</p>
              ...
              <div className="flex items-center gap-1">
                ...
                {(inv.status === "pending" || inv.status === "overdue") && (
                  <button
                    ...
                    className="w-6 h-6 flex items-center justify-center text-gray-300 hover:text-green-600 hover:bg-green-50 rounded-lg transition-colors"
                  >
